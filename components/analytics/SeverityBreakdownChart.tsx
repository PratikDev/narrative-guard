...
const ISSUE_TYPES = Object.keys(AUDIT_ISSUE_TYPE_LABELS) as (keyof typeof AUDIT_ISSUE_TYPE_LABELS)[];
      ...
      filterSlot={
        <Select value={issueType} onValueChange={setIssueType}>
          ...
          <SelectContent>
            ...
            {ISSUE_TYPES.map((it) => (
              <SelectItem key={it} value={it}>{AUDIT_ISSUE_TYPE_LABELS[it]}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      }
    >
      <ChartContainer config={chartConfig} className="mx-auto aspect-square h-70">
        <PieChart>
          ...
          <Pie data={data} dataKey="count" nameKey="severity" innerRadius={55} outerRadius={90} />
