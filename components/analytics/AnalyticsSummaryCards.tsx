	...
	const cards = [
		{
			...
			helper: trend(data.totalAudits, data.prev?.totalAudits),
		},
		{
			label: "Average Score",
			value: data.avgScore,
			helper: trend(data.avgScore, data.prev?.avgScore),
		},
		{
			label: "On Brand",
			value: data.onBrandCount,
			helper: trend(data.onBrandCount, data.prev?.onBrandCount),
		},
		{
			label: "Needs Review",
			value: data.needsReviewCount,
			helper: trend(data.needsReviewCount, data.prev?.needsReviewCount),
		},
		{
			label: "Off Brand",
			value: data.offBrandCount,
			helper: trend(data.offBrandCount, data.prev?.offBrandCount),
		},
		{
			label: "Total Findings",
			value: data.totalFindings,
			helper: trend(data.totalFindings, data.prev?.totalFindings),
		},
