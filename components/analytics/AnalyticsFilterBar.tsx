...
export function AnalyticsFilterBar({ filters, onFiltersChange }: Props) {
	...
	const brands =
		useQuery(api.brand.listBrands, workspaceId ? { workspaceId } : "skip") ??
		[];
	const members =
		useQuery(
	...
	return (
		<div className="flex flex-wrap gap-3 rounded-xl border bg-card p-4">
			<Select
				...
				onValueChange={(v) => update("datePreset", v as DateRangePreset)}
			>
			...
			<Select
				...
				onValueChange={(v) => update("brandId", v as Id<"brands"> | "all")}
			>
			...
			<Select
				...
				onValueChange={(v) => update("contentType", v as ContentType | "all")}
			>
				<SelectTrigger className="w-full sm:w-45">
			...
			<Select
				...
				onValueChange={(v) => update("memberId", v as Id<"users"> | "all")}
			>
				<SelectTrigger className="w-full sm:w-45">
