...
const chartConfig = {
	...
	auditCount: { label: "Audit Count", color: "var(--chart-2)" },
	offBrandCount: { label: "Off Brand", color: "var(--chart-risk)" },
} satisfies ChartConfig;

export function BrandComparisonChart({ filters }: Props) {
	const { workspaceId } = useWorkspace();
	const { fromTs, toTs, contentType, memberId } = filtersToQueryArgs(
		filters,
		workspaceId,
	);
		...
		>
			...
			>
				<BarChart data={data}>
					...
					/>
					<Bar
						dataKey="offBrandCount"
						fill="var(--color-offBrandCount)"
						radius={[4, 4, 0, 0]}
					/>
				</BarChart>
			</ChartContainer>
